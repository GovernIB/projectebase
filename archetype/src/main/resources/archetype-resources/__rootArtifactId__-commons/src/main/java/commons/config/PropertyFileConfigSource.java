...
public class PropertyFileConfigSource implements ConfigSource {
    ...
    private static final String ${projectnameuppercase}_PROPERTIES = "${package}.properties";

    private static final Map<String, String> PROPERTY_MAP;

    static {
        LOG.info("Carregant propietat de configuració: {}", ${projectnameuppercase}_PROPERTIES);

        String propertyFile = System.getProperty(${projectnameuppercase}_PROPERTIES);
        if (propertyFile == null || propertyFile.isEmpty()) {
            throw new RuntimeException("Manca la propietat de sistema " + ${projectnameuppercase}_PROPERTIES);
        }
