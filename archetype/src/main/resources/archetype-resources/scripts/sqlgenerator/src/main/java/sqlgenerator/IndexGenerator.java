...
public class IndexGenerator {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (String l; (l = br.readLine()) != null;) {
                ...
                if (line.startsWith("-- Tabla")) {
                    ...
                    if (lastTable != null) {
                        ...
                        System.out.println(
                                "@org.hibernate.annotations.Table(appliesTo = ${symbol_escape}"" + lastTable + "${symbol_escape}", indexes = {");
                    ...
                    if (line.startsWith("CREATE INDEX")) {
                        ...
                        if (line.startsWith("ON ")) {
                            ...
                            if (indexes.length() != 0) {
                                ...
                                indexes.append(",${symbol_escape}n");
                            }

                            indexes.append("    @Index(name=${symbol_escape}"" + lastIndex + "${symbol_escape}", columnNames = {");
                            for (int j = 0; j < columns.length; j++) {
                                if (j != 0) {
                                    indexes.append(", ");
                                }
                                indexes.append("${symbol_escape}"" + columns[j].trim() + "${symbol_escape}"");
                            }
            ...
            System.out.println("@org.hibernate.annotations.Table(appliesTo = ${symbol_escape}"" + lastTable + "${symbol_escape}", indexes = {");
            System.out.println(indexes.toString());
            System.out.println("})");
            System.out.println();
