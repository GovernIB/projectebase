...
Per provar el plugin:
    ...
    3. Executar l'script "distribucio_create_schema.sql" dins la mateixa base de dades de ${rootArtifactId}.
    ...
    4. Desplegar l'aplicació dins l'EAR
        ...
        El WS per emprar a la regla corresponent de Distribució seria la següent:
            ...
            http://localhost:8080/${rootArtifactId}-distribucio/BackofficeService/BackofficeServicePort

        Si s'ha generat amb el mòdul ${rootArtifactId}-api-interna, dins aquest s'inclou un Domini DISTRIBUCIO
        que retorna les unitats adminsitratives actives creades.

        Per provar el domini dins DISTRIBUCIO cal emprar la URL:

            http://localhost:8080/${rootArtifactId}api/interna/services/distribucio/dominis/unitats
