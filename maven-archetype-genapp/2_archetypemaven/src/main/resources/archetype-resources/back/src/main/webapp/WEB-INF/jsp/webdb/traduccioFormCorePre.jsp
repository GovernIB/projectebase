...
<un:useConstants var="TraduccioFields" className="${package}.model.fields.TraduccioFields"/>
  ...
  <c:set var="__theForm" value="${symbol_dollar}{traduccioForm}"/>
  ...
  <div class="module_content">
    ...
    <c:if test="${symbol_dollar}{not __theForm.view}">    <c:forEach items="${symbol_dollar}{__theForm.hiddenFields}" var="hiddenFieldF" >
      ...
      <c:if test="${symbol_dollar}{gen:hasProperty(__theForm.traduccio,hiddenField)}">
        <form:errors path="traduccio.${symbol_dollar}{hiddenField}" cssClass="errorField alert alert-error" />
        <form:hidden path="traduccio.${symbol_dollar}{hiddenField}"/>
      </c:if>
    </c:forEach>
