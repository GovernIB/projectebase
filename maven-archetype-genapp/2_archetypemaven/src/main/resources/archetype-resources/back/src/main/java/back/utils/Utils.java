  ...
  protected static String getSortIcons(BaseFilterForm baseFilter, 
    ...
    if (baseFilter.isVisibleOrderBy() && theField != null) {
      ...
      if (orderedBythisfield) {
        ...
        html = "<span style=${symbol_escape}"cursor:row-resize${symbol_escape}" onClick=${symbol_escape}"javascript:executeOrderBy('"
            + field
            + "', "
            + !isOrderedAsc
            + ");${symbol_escape}""
            ...
            + "${symbol_escape}" >" + getText(code) + "<i class=${symbol_escape}""
            + (isOrderedAsc ? "icon-chevron-up" : "icon-chevron-down")
            + "${symbol_escape}"></i></span>";
      } else {
        html = "<span style=${symbol_escape}"cursor:row-resize${symbol_escape}" onclick=${symbol_escape}"javascript:executeOrderBy('" + field + "', true);${symbol_escape}" "
            + " title=${symbol_escape}"" + I18NUtils.tradueix("genapp.form.sort.asc") + "${symbol_escape}">"
            + getText(code) + "<i class=${symbol_escape}"icon-resize-vertical${symbol_escape}"></i></span>";
      }
      return html;
    } else {
      return getText(code);
